...
: Node("velocity_smoother", node_options)
{
  ...
  time_keeper_ =std::make_shared<autoware::universe_utils::TimeKeeper>(debug_processing_time_detail_);
  ...
  // subscribers
  sub_current_trajectory_ = create_subscription<Trajectory>("~/input/trajectory", 1, std::bind(&VelocitySmootherNode::onCurrentTrajectory, this, _1));
  
  publish_debug_trajs_ = false; //declare_parameter<bool>("publish_debug_trajs");
  /*
  // debug
  ...
  pub_trajectory_vel_lim_ =create_publisher<Trajectory>("~/debug/trajectory_external_velocity_limited", 1);
  pub_trajectory_latacc_filtered_ =create_publisher<Trajectory>("~/debug/trajectory_lateral_acc_filtered", 1);
  pub_trajectory_steering_rate_limited_ =create_publisher<Trajectory>("~/debug/trajectory_steering_rate_limited", 1);
...
bool VelocitySmootherNode::checkData() const
{
  if (!current_odometry_ptr_ || !base_traj_raw_ptr_ ) {
    RCLCPP_DEBUG(
      get_logger(), "wait topics : current_vel = %d, base_traj = %d",
      (bool)current_odometry_ptr_, (bool)base_traj_raw_ptr_);
    return false;
  }
...
void VelocitySmootherNode::initCommonParam()
{
  ...
  p.margin_to_insert_external_velocity_limit =declare_parameter<double>("margin_to_insert_external_velocity_limit");
  ...
  p.post_resample_param.max_trajectory_length =declare_parameter<double>("post_max_trajectory_length");
  p.post_resample_param.min_trajectory_length =declare_parameter<double>("post_min_trajectory_length");
  ...
  p.post_resample_param.dense_min_interval_distance =declare_parameter<double>("post_dense_min_interval_distance");
  p.post_resample_param.sparse_resample_dt = declare_parameter<double>("post_sparse_resample_dt");
  p.post_resample_param.sparse_min_interval_distance =declare_parameter<double>("post_sparse_min_interval_distance");
  p.algorithm_type = getAlgorithmType(declare_parameter<std::string>("algorithm_type"));

  p.plan_from_ego_speed_on_manual_mode =declare_parameter<bool>("plan_from_ego_speed_on_manual_mode");
}
...
VelocitySmootherNode::AlgorithmType VelocitySmootherNode::getAlgorithmType(const std::string & algorithm_name) const
{
  ...
  const TrajectoryPoints & traj_input) const
{
  ...
  */
  ...
  // Apply velocity to approach stoping point
  ...
  TrajectoryPoints & traj_smoothed) const
{
  ...
  //publishClosestVelocity(traj_resampled, current_odometry_ptr_->pose.pose, debug_closest_max_velocity_);
...
Trajectory VelocitySmootherNode::toTrajectoryMsg(const TrajectoryPoints & points, const std_msgs::msg::Header * header) const
{
