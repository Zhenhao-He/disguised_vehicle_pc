...
: Node("planning_validator", options)
{
  ...
  pub_processing_time_ms_ = create_publisher<Float64Stamped>("~/output/planning_validator/processing_time_ms", 1);
  pub_status_ = create_publisher<PlanningValidatorStatus>("~/output/validation_status", 1);
  pub_planning_status_ = create_publisher<RouteState>("~/output/planning_state", 1);
...
{
  //到达终点，发布ARRIVED
  current_parking_state_ = sub_parking_state_.takeData();
  if(current_parking_state_->data==true){
    planning_state_.state = RouteState::ARRIVED;}
  else if(isAllValid(validation_status_)){
    planning_state_.state = RouteState::SET;}
  else{
    planning_state_.state = RouteState::UNSET;}
  pub_planning_status_->publish(planning_state_);
