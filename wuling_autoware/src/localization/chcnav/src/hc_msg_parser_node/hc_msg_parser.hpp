...
public:
    ...
    hc__msg_parser_node(unsigned int hz, hc__device_connector *device_connector, std::shared_ptr<rclcpp::Node> nh, std::shared_ptr<rclcpp::Node> private_nh, string node_name)
    {
        if (NULL != nh && NULL != private_nh && NULL != device_connector)
        {
            ...
            // 初始化nmea协议话题
            this->nmea_puber = this->nh->create_publisher<msg_interfaces::msg::String>("~/output/chcnav/nmea_sentence", 1000); // a public topic
            //this->nmea_msg.header = 0;
            this->nmea_msg.header.frame_id = node_name;

            // 初始化华测自定义协议话题
            this->hc_puber = this->nh->create_publisher<msg_interfaces::msg::HcSentence>("~/output/chcnav/hc_sentence", 1000); // a public topic
