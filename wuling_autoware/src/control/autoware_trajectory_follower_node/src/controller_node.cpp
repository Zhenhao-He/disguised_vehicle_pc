...
Controller::Controller(const rclcpp::NodeOptions & node_options) : Node("controller", node_options)
{
  ...
  pub_processing_time_lat_ms_ =
    create_publisher<Float64Stamped>("~/output/lateral/processing_time_ms", 1);
  pub_processing_time_lon_ms_ =
    create_publisher<Float64Stamped>("~/output/longitudinal/processing_time_ms", 1);
  debug_marker_pub_ =
...
{
  if (sub_accel_.takeData()) {current_accel_ptr_ = sub_accel_.takeData();}
  else{return false;}

  if (sub_steering_.takeData()) {current_steering_ptr_ = sub_steering_.takeData();}
  else{return false;}

  if (sub_ref_path_.takeData()) {current_trajectory_ptr_ = sub_ref_path_.takeData();}
  else{return false;}

  if (sub_odometry_.takeData()) {current_odometry_ptr_ = sub_odometry_.takeData();}
  else{return false;}
...
boost::optional<trajectory_follower::InputData> Controller::createInputData()
{
  ...
  input_data.current_operation_mode.mode= autoware_adapi_v1_msgs::msg::OperationModeState::AUTONOMOUS;
