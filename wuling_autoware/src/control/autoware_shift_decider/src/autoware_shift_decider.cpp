...
void ShiftDecider::onTimer()
{
  ...
  if (!autoware_state_ || !control_cmd_ || !current_gear_ptr_) {
    if (!autoware_state_) {
      RCLCPP_WARN(this->get_logger(), "Autoware state not received.");
    }
    if (!control_cmd_) {
      RCLCPP_WARN(this->get_logger(), "Control command not received.");
    }
    if (!current_gear_ptr_) {
      RCLCPP_WARN(this->get_logger(), "Current gear not received.");
    }
    return;
  }
