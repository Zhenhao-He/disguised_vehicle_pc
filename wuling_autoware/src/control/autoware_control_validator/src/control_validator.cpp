...
: Node("control_validator", options), validation_params_(), vehicle_info_()
{
  ...
  pub_control_status_ = create_publisher<MotionState>("~/output/control_state", 1);
  ...
  const Odometry & kinematics)
{
  ...
  if(is_all_valid(validation_status_)){
    control_state_.state= MotionState::MOVING;
  }else{
    control_state_.state= MotionState::STOPPED;
  }
  pub_control_status_->publish(control_state_);
}
...
void ControlValidator::display_status()
{
  ...
  warn(
    ...
    "predicted trajectory deviates too much from the planning trajectory's path!!");

  warn(
    s.is_valid_velocity_deviation,
    "predicted trajectory deviates too much from the planning trajectory's velocity!!");
}
