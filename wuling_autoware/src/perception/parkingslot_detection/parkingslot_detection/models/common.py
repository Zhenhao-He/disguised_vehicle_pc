...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        b, ch, h, w = im.shape  # batch, channel, height, width 1 3 448 640
...
class FeatureMapTransform(nn.Module):
    def __init__(self,change_=0):
        super(FeatureMapTransform, self).__init__()
        
        # Define layers for transforming feature maps
        self.conv1 = nn.Conv2d(144, 64, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(32, 16, kernel_size=3, padding=1)
        self.conv4 = nn.Conv2d(16, 6, kernel_size=3, padding=1)
        self.change=change_
    def forward(self, xs):
        # Input xs is assumed to have shape (batch_size, 3, 4, H, W),
        # where xs[0] and xs[1] are lists of tuples representing feature maps
        if self.change==1:
            outputs = []
            for sample in xs:
                transformed_maps = []
                for feature_map in sample:
                    # Apply convolutions to downsize spatial dimensions
                    x = F.relu(self.conv1(feature_map))
                    x = F.relu(self.conv2(x))
                    x = F.relu(self.conv3(x))
                    
                    # Upsample to target size (1, 16, 16)
                    x = F.interpolate(x, size=(16, 16), mode='bilinear', align_corners=False)
                    
                    transformed_maps.append(x)
                
                outputs.append(transformed_maps)
