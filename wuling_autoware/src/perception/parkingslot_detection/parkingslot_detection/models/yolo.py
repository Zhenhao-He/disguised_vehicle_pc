...
class Detect(nn.Module):
    ...
    def __init__(self, nc=2, ch=(), inplace=True):  # detection layer
        super().__init__()
...
class DualDDetect(nn.Module):
    ...
    def __init__(self, nc=2, ch=(), inplace=True):  # detection layer ch=[31, 34, 37, 16, 19, 22]
        super().__init__()
        self.nc = nc  # number of classes
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc+2, 1)) for x in ch[:self.nl])
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3, g=4), nn.Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x in ch[self.nl:])
        self.cv5 = nn.ModuleList(
            nn.Sequential(Conv(x, c5, 3), Conv(c5, c5, 3), nn.Conv2d(c5, self.nc+2, 1)) for x in ch[self.nl:])
        # self.cv2 = nn.ModuleList(
        #     nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3, g=4), nn.Conv2d(c2, 4 * self.reg_max, 1, groups=4)) for x in ch[:self.nl])
        # self.cv3 = nn.ModuleList(
        #     nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, self.nc+2, 1)) for x in ch[:self.nl])
        # self.cv4 = nn.ModuleList(
        #     nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3, g=4), nn.Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x in ch[self.nl:])
        # self.cv5 = nn.ModuleList(
        #     nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nc+2, 1)) for x in ch[self.nl:])
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
    ...
    def forward(self, x):
        ...
        box, cls ,orien= torch.cat([di.view(shape[0], self.no, -1) for di in d1], 2).split((self.reg_max * 4, self.nc,2), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        box2, cls2,orien2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc,2), 1)
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = [torch.cat((dbox, cls.sigmoid(),orien), 1), torch.cat((dbox2, cls2.sigmoid(),orien2), 1)]
        # y = [torch.cat((dbox, cls,orien), 1), torch.cat((dbox2, cls2,orien2), 1)]
        return y if self.export else (y, [d1, d2])
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)#使输出维度能被8整除
        ...
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module m(*args)
