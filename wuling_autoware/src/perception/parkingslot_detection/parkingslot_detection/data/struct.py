...
MarkingPoint = namedtuple('MarkingPoint', ['x', 'y', 'direction', 'shape','h','w'])
...
def detemine_point_shape(point, vector):
    ...
    sin_value = point[7].item()
    cos_value = point[6].item()
    point_direction = math.atan2(sin_value, cos_value)
    
    if point[5] < 0.5:
        if direction_diff(vec_direct, point_direction) < config.BRIDGE_ANGLE_DIFF:
            return PointShape.t_middle
        if direction_diff(vec_direct_up, point_direction) < config.SEPARATOR_ANGLE_DIFF:
            return PointShape.t_up
        if direction_diff(vec_direct_down, point_direction) < config.SEPARATOR_ANGLE_DIFF:
...
def calc_point_squre_dist(point_a, point_b):
    ...
    a_x = (point_a[..., 0] + point_a[..., 2]) / 1280
    a_y = (point_a[..., 1] + point_a[..., 3]) / 1280
    b_x = (point_b[..., 0] + point_b[..., 2]) / 1280
    b_y = (point_b[..., 1] + point_b[..., 3]) / 1280
    distx = a_x - b_x
    disty = a_y - b_y
...
def calc_point_squre_dist_eval(point_a, point_b):
    """Calculate distance between two marking points."""
...
def match_marking_points_eval(point_a, point_b):
    """Determine whether a detected point match ground truth."""
    dist_square = calc_point_squre_dist_eval(point_a, point_b)
    angle = calc_point_direction_angle(point_a, point_b)
    if point_a.shape > 0.5 and point_b.shape < 0.5:
        return False
    if point_a.shape < 0.5 and point_b.shape > 0.5:
